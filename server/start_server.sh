...
python manage.py runserver 0.0.0.0:3346 > "$DJANGO_LOG" 2>&1 &
DJANGO_PID=$!

# Start the Celery worker
echo "Starting Celery worker..."
celery -A server worker -l INFO > "$CELERY_WORKER_LOG" 2>&1 &
CELERY_WORKER_PID=$!

# Start the Celery beat scheduler
echo "Starting Celery beat..."
celery -A server beat -l INFO > "$CELERY_BEAT_LOG" 2>&1 &
CELERY_BEAT_PID=$!
